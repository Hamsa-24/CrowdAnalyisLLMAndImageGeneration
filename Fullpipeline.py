...
def process_video(video_path):
    ...
    return json_data, peoplecount

def enhance_json_data(json_data, peoplecount):
    enhanced_json_data = []
...
# Process the video only for the first question and save the enhanced JSON data
if uploaded_video is not None:
    processed_data_dir = 'processed_data'
    enhanced_json_file_path = os.path.join(processed_data_dir, 'enhanced_data.json')
    ...
    if not st.session_state.video_processed:
        st.write("Processing video and generating enhanced JSON data...")
        json_data, peoplecount = process_video(video_path)
        enhanced_json_data = enhance_json_data(json_data, peoplecount)
        ...
        st.write("Video processed and enhanced JSON data saved.")
    else:
        st.write("Using previously saved enhanced JSON data.")

    # Set up LLM and RAG only once
    if "qa_chain" not in st.session_state:
        st.session_state.qa_chain = setup_llm_and_rag(enhanced_json_file_path)
    ...
    if user_query:
        result = st.session_state.qa_chain({'question': user_query, 'chat_history': chat_history})
